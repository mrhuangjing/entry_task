...
function Detail (props) {
  ...
  async function queryEventInfo (id) {
    try {
      ...
      const createTime = moment(new Date(res.create_time))
      const now = moment(new Date())
      const diff = now.diff(createTime, 'day')
  ...
  async function queryCommentList (id) {
    try {
      ...
      const list = res.map(el => {
        ...
        const createTime = moment(new Date(el.create_time))
        const now = moment(new Date())
        const days = now.diff(createTime, 'day')
        const hours = now.diff(createTime, 'hours')
        const mins = now.diff(createTime, 'minutes')
        const secs = now.diff(createTime, 'seconds')
        if (days >= 1) {
  ...
  return (
    <>
      <Top />
      <div className='detail' onScroll={handleScroll} style={{ height: `${window.innerHeight - 50}px` }}>
        ...
        <div className='detail_related'>
          <div className='detail_related_going' style={{ minHeight: goingAll ? 'none' : '3.025rem', height: goingAll ? 'auto' : '3.025rem' }}>
            {
                          !goingAll && participants.length > 6 && (<div className='detail_related_going_btn on' onClick={handleGoingBtnClick} />)
                      }
                      ...
                      comments.map((el, index) => {
                        return (
                          <div className='detail_comments_item' key={index}>
                            ...
                            <div className='detail_comments_item_back' onClick={() => handleCommentPerson(index)} />
                  ...
                  showCommentPanel
                    ? (
                      <div className='detail_bottom'>
                        ...
                        <div className='detail_bottom_send' onClick={handleSendComment} />
                      </div>
                      ) : (<div className='detail_bottom'>
                        <div style={{ margin: '0 auto', display: 'flex' }}>
                        <div className='detail_bottom_left'>
                          <div className='detail_bottom_left_comment' onClick={() => setShowCommentPanel(true)} />
                          <div className='detail_bottom_left_like' onClick={handleLike} />
                        </div>
